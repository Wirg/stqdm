...
def test_scope_manager__get_default_config_does_not_expose_mutable_internal_config():
    scope_manager = ScopeManager({"foo": "initial"})

    retrieved_config = scope_manager.get_default_config()
    retrieved_config["foo"] = "mutated"
...
def test_scope_manager__get_current_scope_config_does_not_expose_mutable_internal_scope():
    ...
    with scope_manager.scope({"foo": "initial"}):
        retrieved_config = scope_manager.get_current_scope_config()
        retrieved_config["foo"] = "mutated"
...
def test_scope_manager__mutating_yielded_scope_mapping_does_not_change_inner_inheritance():
    scope_manager = ScopeManager({})

    with scope_manager.scope({"bar_format": "{desc}", "desc": "outer"}) as outer_scope:
        with scope_manager.scope({"desc": "inner"}):
            outer_scope["bar_format"] = None
